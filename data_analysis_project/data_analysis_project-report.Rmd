...
Multi-Ethnic Study of Atherosclerosis (MESA) used advanced magnetic resonance
imaging (MRI) measurement tools to evaluate the prevalence and progression of
...
(4-5). Our first aim is to use cross-sectional MESA data to characterize the
association between CRP and left ventricular mass (LVM). Our second aim is to
further investigate the complex social, demographic, biological, and behavioral
mechanisms through which inflammation contributes to LVH, thus identifying
predictive factors and enhancing interventions for averting global CVD deaths.
...
&+ crp[\beta_9(sex) + \beta_10(bmi) + \beta_11(age) + \beta_12(sbp) + \beta_{13-15}(race)]
\end{aligned}
$$
...
The distribution of CRP is heavily right skewed, so in our models we use
the $log_2$ transformation of CRP. All statistical significance tests are performed
...
meanstandard deviation (SD) age was $62 \pm 10$ years and $53\%$ were female.
...
In Aim 1, based on a multiple linear regression model adjusted for sex and BMI,
...
evaluate the relationship between LVM and $log_2(CRP)$. An association between LVM
and $log_2(CRP)$ was not readily apparent among the combined sex cohort, but when
stratified by sex, a first-order trend of positive association between higher
$log_2(CRP)$ and higher LVM was evident among both sexes (Figure 1),
suggesting that sex is likely a confounder of the LVM-$log_2(CRP)$ relationship.
...
analysis. Similar slopes for the race-specific associations suggested no
noticeable differences across race categories. Thus, race did not appear to
modify the LVM-$log_2(CRP)$ association.
...
systems in the body, which can directly affect CRP levels and directly affect
LVM. Our multivariate linear regression analyses suggest BMI is an effect
modifier of the association between CRP and LVH, which is also biologically
plausible, given that higher BMI is often associated with greater adiposity, a
hormone-releasing tissue known to be generally associated with higher levels of
inflammation in the body and also known to be associated with higher LVM.
However, it is likely that LVH is also associated with complex mechanisms
involving multiple social, demographic, biological, and behavioral risk factors
that are beyond the scope of our data analysis, and more studies are needed to
continue to advance our understanding of the association that such variables
have on LVH, which in turn could help focus interventions to reduce CVD related

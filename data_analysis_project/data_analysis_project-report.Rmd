...
knitr::kable(stbl_cont, booktabs = TRUE, digits = 3,
  caption = "MESA summary table (all continous variables)"
)
```

```{r}
stbl_cont_used <- mesa %>%
  select(lvmass, crp1, log2_crp, bmi1c, age, sbp1c) %>%
  summary_table()

knitr::kable(stbl_cont_used, booktabs = TRUE, digits = 3,
  caption = "MESA summary table (used continous variables)"
)
```
...
stbl_cat <-
  ...
  mutate(variable = c("Sex", "Ethnicity", "Htn. medicaton", "Htn. status", "Smoking status", "Treated diabetes", "Study site", "Eduction", "Income"))
...
crude_lm <- mesa %>%
  filter(!is.na(lvmass)) %>%
  regress("mean", lvmass ~ log2_crp + sex + bmi1c, data = .)
...
```{r}
full_lm <- mesa %>%
  filter(!is.na(lvmass), f_cig_use != "(Missing)", !is.na(sbp1c)) %>%
  regress(
    "mean",
    lvmass ~ log2_crp + sex + bmi1c + age + factor(f_race) + sbp1c,
    data = .
  )
```
...
full_lm %>%
  factor_inf_table() %>%
  mutate(`Pr(>F)` = format(`Pr(>F)`, digits = 3)) %>%
  knitr::kable(booktabs = TRUE, digits = 3,
    caption = "Full model (no interaction terms)"
  )
```
...
```{r}
full_lm_int <- mesa %>%
  filter(!is.na(lvmass), !is.na(sbp1c)) %>%
  regress(
    "mean",
    lvmass ~ log2_crp * (sex + bmi1c + age + factor(f_race) + sbp1c),
    data = .
...
full_lm_int %>%
  ...
  mutate(`Pr(>F)` = format(`Pr(>F)`, digits = 3)) %>%
  knitr::kable(booktabs = TRUE, digits = 3,
    caption = "Full model (with interaction terms)"
  )
```

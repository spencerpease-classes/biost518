...
Rainfall should be a confounder
...
$$ E[mortality | log(NOx), rain] = \beta_{0} + \beta_{1} \times log(NOx) + \beta_{2} \times rain $$

```{r}
mort_nox_rain_lm <- regress("mean", mort ~ log_nox + rain, data = smsa)

mort_nox_rain_inf_tbl <- mort_nox_rain_lm %>%
  inference_table(paramter_names = c("log(NOx)", "Rain")) %>%
